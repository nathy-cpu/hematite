...
| `F9` Cursor And Read-Path Rewrite | Rebuild B-tree navigation around the new slotted-page format | `In Progress` | Hot B-tree point-lookups and cursor descent already use the pager's shared-page path, and tree open/validation/page-id/stats walkers now read slotted pages through shared-page lazy nodes instead of forcing owned-page clones on read-only traversals | Continue moving overflow validation and any remaining read-only inspection helpers onto the same cheaper shared-page path |
