//! Mutable B-tree implementation over ordered byte keys and opaque byte values.
//!
//! `BTreeIndex` owns the mutation algorithms for a single tree root. It is responsible for:
//! - recursive search down separator keys;
//! - leaf insertion and replacement;
//! - node splitting with root promotion;
//! - delete-time underflow detection plus borrow/merge rebalancing;
//! - reporting whether a mutation changed the logical root page id.
//!
//! The implementation is B+ tree-like:
//! - leaf pages store the actual key/value pairs;
//! - internal pages store separator keys and child page ids;
//! - separator keys are the minimum keys of right subtrees.
...
impl BTreeIndex {
    ...
    pub(crate) fn check_root_underflow(&mut self, pager: &mut Pager) -> Result<Option<PageId>> {
